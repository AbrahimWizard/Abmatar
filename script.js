...
function throttle(func, limit) {
  ...
  class BioLuminescence {
    ...
    animate() {
      ...
      for (let particle of this.particles) {
        // Update position with a sin/cos based motion.
   ...
   * (Optional: Only runs if a container with ID "knowledge-web" exists.)
   */
  class NeuralWeb {
    /**
    ...
    initConnections() {
      ...
      // Create a connection for every pair of nodes.
  ...
  class ThemeToggle {
    ...
    toggleTheme() {
      ...
      // Toggle between "neural-future" and "neural-dark".
   ...
   * Instantiates all modules and sets up global event listeners.
   */
  document.addEventListener("DOMContentLoaded", () => {
    ...
    // Instantiate BioLuminescence on hero canvas.
    const heroCanvas = document.getElementById("hero-canvas");
    if (heroCanvas) new BioLuminescence(heroCanvas);
    
    // Instantiate NeuralWeb if container exists (optional).
    ...
    // Instantiate SingularityEngine on portfolio section for drag interactions.
    const portfolioSection = document.querySelector(".portfolio-section");
    if (portfolioSection) new SingularityEngine(portfolioSection);
    
    // Instantiate MatrixTerminal for digital rain in footer.
    ...
    // Instantiate ThemeToggle for the theme switch.
    const themeButton = document.getElementById("theme-toggle");
    if (themeButton) new ThemeToggle(themeButton);
    
    // Setup AI Core Dialog management.
    const aiDialog = document.getElementById("ai-core");
    if (aiDialog) new AIDialog(aiDialog);
    
    // Global scroll event: Update CSS variable for scroll position.
